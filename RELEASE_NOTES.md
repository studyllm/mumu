...
- ✨ **主界面新增提醒时间预览**（T36）：底部三行紧凑堆叠（休息 / 眼药水 / 热敷），每行显示上次提醒时间 + 距下次倒计时。未启用的提醒整行隐藏。
- 🐛 **修下次提醒倒计时不递减**（T37）：客户端每 1s 自减显示，避免后端 1Hz polling 把本地值重置成固定常量。
- 🐛 **修强提醒永远不触发**（T38）：scheduler 启动时 `last_strong_at=None` 会让 `elapsed=now-now=0`，改为 tick 首次进入工作时段时把 `last_*_at` 初始化为 now，恢复"等满 interval 才触发"语义。
...
- ✅ **提醒时间预览**：主窗口底部新增三行紧凑块，显示上次提醒时间 + 距下次倒计时（休息 / 眼药水 / 热敷）
